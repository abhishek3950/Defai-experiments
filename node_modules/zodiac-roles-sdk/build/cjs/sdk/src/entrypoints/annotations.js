...
exports.resolveAnnotation = exports.processAnnotations = void 0;
...
const processAnnotations = async (permissions, annotations, options = {}) => {
    const { fetchPermissions = defaultJsonFetch, fetchSchema = defaultJsonFetch, } = options;
    const cache = new Map();
    const cachedFetchSchema = (url) => {
    ...
    const presets = await Promise.all(annotations.map(async (annotation) => {
        try {
            return await (0, exports.resolveAnnotation)(annotation, {
                fetchPermissions,
    ...
    const confirmedPresets = presets.filter((preset) => {
        if (!preset)
            return false;
        let targetsWithPresetApplied = [];
        try {
            const { targets } = (0, zodiac_roles_sdk_1.processPermissions)([
                ...permissions,
                ...preset.permissions,
            ]);
        ...
        return (0, zodiac_roles_sdk_1.diffTargets)(targetsWithPresetApplied, targets).length === 0;
    });
    // Calculate remaining permissions that are not part of any preset
    const { targets: targetsViaPresets } = (0, zodiac_roles_sdk_1.processPermissions)(confirmedPresets.flatMap((preset) => preset.permissions));
    const remainingTargets = (0, zodiac_roles_sdk_1.splitTargets)(targets, targetsViaPresets);
    const remainingPermissions = (0, zodiac_roles_sdk_1.reconstructPermissions)(remainingTargets);
    // Extra safety check: assert that the final set of targets remains unchanged
    const { targets: finalTargets } = (0, zodiac_roles_sdk_1.processPermissions)([
        ...confirmedPresets.flatMap((preset) => preset.permissions),
        ...remainingPermissions,
    ]);
    const d1 = (0, zodiac_roles_sdk_1.diffTargets)(finalTargets, targets);
    const d2 = (0, zodiac_roles_sdk_1.diffTargets)(targets, finalTargets);
    if (d1.length !== 0 || d2.length !== 0) {
        throw new Error("The processed results leads to a different set of targets.");
    }
    return { presets: confirmedPresets, permissions: remainingPermissions };
};
exports.processAnnotations = processAnnotations;
const resolveAnnotation = async (annotation, { fetchPermissions, fetchSchema, }) => {
    const normalizedUri = normalizeUri(annotation.uri);
    const [permissions, schema] = await Promise.all([
        fetchPermissions(normalizedUri).catch((e) => {
    ...
    const { serverUrl, path, query } = parseUri(normalizedUri, schema, annotation.schema);
    const { operation, params, query: parsedQuery, } = matchAction(schema, { path, query });
    if (!operation) {
...
const parseUri = (annotationUrl, schema, schemaUrl) => {
    ...
    const serverUrls = !schema.document.servers || schema.document.servers.length === 0
        ? ["/"]
        : schema.document.servers.map((server) => server.url);
    const absoluteServerUrls = serverUrls.map((serverUrl) => new URL(serverUrl, schemaUrl).href);
    const matchingServerUrl = absoluteServerUrls.find((serverUrl) => new URL(annotationUrl, serverUrl).href.startsWith(serverUrl));
    if (!matchingServerUrl) {
        throw new Error(`Annotation url ${annotationUrl} is not within any server url declared in the schema ${schemaUrl}`);
    }
    // extract the pathname after the server base path and the query string
    const pathAndQuery = new URL(annotationUrl, matchingServerUrl).href.slice(matchingServerUrl.length);
...
const matchAction = (schema, { path, query }) => {
...
const defaultJsonFetch = async (url) => {
    const res = await fetch(url);
    return await validateJsonResponse(res);
};
async function validateJsonResponse(res) {
    if (res.ok) {
