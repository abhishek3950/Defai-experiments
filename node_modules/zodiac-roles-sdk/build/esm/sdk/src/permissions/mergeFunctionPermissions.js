 ...
 */
export const mergeFunctionPermissions = (permissions) => permissions.reduce((result, entry) => {
    ...
    const matchingEntry = result.find((existingEntry) => targetId(existingEntry) === targetId(coercedEntry));
    if (!matchingEntry) {
...
const mergeConditions = (a, b) => {
    ...
    if (!a.condition || !b.condition)
        return undefined;
    const aBranches = a.condition.operator === Operator.Or ? a.condition.children : [a.condition];
    const bBranches = b.condition.operator === Operator.Or ? b.condition.children : [b.condition];
    return {
