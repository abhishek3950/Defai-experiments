...
export const processPermissions = (permissions) => {
    ...
    const functionScopedTargets = Object.entries(groupBy(functionPermissions, (entry) => entry.targetAddress)).map(([targetAddress, allowFunctions]) => ({
...
const assertNoWildcardScopedIntersection = (permissions) => {
    ...
    const scopedTargets = new Set(permissions.filter(isFunctionScoped).map((entry) => entry.targetAddress));
    const intersection = [
...
const assertNoDuplicateAllowFunction = (permissions) => {
    ...
    const counts = allowFunctions.reduce((result, item) => ({ ...result, [item]: (result[item] || 0) + 1 }), {});
    const duplicates = [
