import { isHexString, ParamType } from "ethers";
 ...
 */
export const matches = (scoping) => (abiType) => {
    ...
    return {
        ...
        children: conditions.map((condition, index) => condition ||
            describeStructure(abiType.baseType === "tuple"
                ? abiType.components[index]
                : abiType.arrayChildren)),
    };
};
const calldataMatchesScopings = (scopings, abiTypes, selector) => (abiType) => {
    ...
    const matchesCondition = {
        ...
        operator: Operator.Matches,
        children: conditions.map((condition, index) => condition || describeStructure(paramTypes[index])),
    };
    if (selector) {
        if (selector.length !== 10) {
            throw new Error(`Selector must be exactly 4 bytes long, got: ${selector.length}`);
        }
        return and(() => matchesCondition, bitmask({ mask: "0xffffffff", value: selector }))(ParamType.from("bytes"));
    }
...
const calldataMatchesFunctionPermission = (functionPermission) => (abiType) => {
    ...
    if (condition) {
        if (condition.operator !== Operator.Matches ||
            ...
            throw new Error(`calldataMatches expects a function permission with an \`Operator.matches\`, \`ParamType.Calldata\` condition, got: \`Operator.${Operator[condition.operator]}\`, \`ParamType.${ParameterType[condition.paramType]}\``);
        }
    }
    ...
    return (condition ? and(() => condition, selectorCondition) : selectorCondition)(ParamType.from("bytes"));
};
export const calldataMatches = (scopingsOrFunctionPermission, abiTypes, selector) => {
    return abiTypes
 ...
 **/
export const abiEncodedMatches = (scopings, abiTypes) => (abiType) => {
    const paramTypes = abiTypes.map((abiType) => ParamType.from(abiType));
    // only supported at the top level or for bytes type params
    if (abiType && abiType.type !== "bytes") {
 ...
 * @returns
 */
export function mapScoping(scoping, abiType) {
...
const assertCompatibleParamTypes = (conditions, typeOrTypes) => {
    conditions.forEach((condition, index) => {
        ...
        if (expectedType === ParameterType.Dynamic &&
        ...
        throw new Error(`Condition for field ${fieldReference} has wrong paramType \`${ParameterType[scopedType]}\` (expected: \`${ParameterType[expectedType]}\`)`);
    });
};
/**
