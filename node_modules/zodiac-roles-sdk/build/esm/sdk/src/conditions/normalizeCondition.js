...
export const isNormalized = (condition) => "$$id" in condition && !!condition.$$id;
const addId = (condition) => {
    condition.$$id = calculateConditionId(condition);
    return condition;
};
export const stripIds = (condition) => {
...
const pruneTrailingPass = (condition) => {
    if (!condition.children)
        return condition;
    if (condition.operator !== Operator.Matches)
        return condition;
    const isDynamicTuple = condition.paramType === ParameterType.Tuple && isDynamicParamType(condition);
    // We must not apply this to static tuples since removing Static Pass nodes would cause word shifts in the encoding.
    const canPrune = condition.paramType === ParameterType.Calldata ||
    ...
    let prunedChildren = condition.children.slice(0, keepChildrenUntil + 1);
...
const flattenNestedLogicalConditions = (condition) => {
    if (condition.operator === Operator.And ||
        condition.operator === Operator.Or) {
        if (!condition.children)
            return condition;
        const flattenedChildren = condition.children.flatMap((child) => child.operator === condition.operator ? child.children || [] : [child]);
        condition.children = flattenedChildren;
    }
    return condition;
};
/** remove duplicate child branches in AND/OR/NOR */
const dedupeBranches = (condition) => {
    if (condition.operator === Operator.And ||
        condition.operator === Operator.Or ||
...
const unwrapSingleBranches = (condition) => {
    if (condition.operator === Operator.And ||
        condition.operator === Operator.Or) {
...
const normalizeChildrenOrder = (condition) => {
    if (condition.operator === Operator.And ||
        condition.operator === Operator.Or ||
        condition.operator === Operator.Nor) {
        if (!condition.children)
        ...
        const moveToFront = condition.children.filter((child) => child.paramType === ParameterType.Calldata ||
...
const pushDownLogicalConditions = (condition) => {
    if (condition.operator === Operator.And ||
        condition.operator === Operator.Or) {
        ...
        if (first.operator === Operator.Matches) {
            ...
            if (hingeIndex === null)
                return condition;
            const children = first.children?.map((child, i) => i !== hingeIndex
                ? child
                : {
            ...
            if (children?.some((child) => child.children?.some((c) => c === undefined))) {
                throw new Error("This case has not been implemented yet. Please report this issue at https://github.com/gnosisguild/zodiac-modifier-roles/issues");
            }
            updatedCondition = {
                ...first,
                children: children,
...
const findMatchesHingeIndex = (conditions) => {
    ...
    for (const other of others) {
        ...
        if (!other.children)
            throw new Error("empty children");
        for (let i = 0; i < Math.max(first.children.length, other.children.length); i++) {
 ...
 **/
const findAndHingeIndices = (conditions) => {
    ...
    const uniqueChildrenIds = allChildrenIdsSets.map((ids) => difference(ids, commonChildrenIds));
...
const intersection = (...sets) => {
    ...
    return new Set(Array.from(first).filter((element) => others.every((set) => set.has(element))));
};
const difference = (a, b) => Array.from(a).filter((element) => !b.has(element));
const deleteUndefinedFields = (condition) => {
    if ("children" in condition && !condition.children)
        delete condition.children;
    if ("compValue" in condition && !condition.compValue)
        delete condition.compValue;
    return condition;
};
const isDynamicParamType = (condition) => {
...
const conditionsEqual = (a, b) => (!a || a.$$id) === (!b || b.$$id);
