...
export const checkConditionIntegrity = (condition) => {
    checkConsistentChildrenTypes(condition);
    checkConditionIntegrityRecursive(condition);
};
export const checkRootConditionIntegrity = (condition) => {
    const rootType = checkConsistentChildrenTypes(condition);
    if (rootType !== ParameterType.Calldata) {
        throw new Error(`Root param type must be \`Calldata\`, got \`${ParameterType[rootType]}\``);
    }
    checkConditionIntegrityRecursive(condition);
};
/**
...
export const checkParameterTypeCompatibility = (left, right) => {
    if (right === ParameterType.None)
        return;
    if (right === left)
        return;
    if (right === ParameterType.Dynamic &&
        (left === ParameterType.Calldata || left === ParameterType.AbiEncoded)) {
        return;
    }
    if ((right === ParameterType.Calldata || right === ParameterType.AbiEncoded) &&
        left === ParameterType.Dynamic) {
        throw new Error(`Mixed children types: \`${ParameterType[right]}\` must appear before \`${ParameterType[left]}\``);
    }
    throw new Error(`Inconsistent children types (\`${ParameterType[left]}\` and \`${ParameterType[right]}\`)`);
};
...
const checkParamTypeIntegrity = (condition) => {
    ...
    if (!compatibleTypes.includes(condition.paramType)) {
        throw new Error(`\`${Operator[condition.operator]}\` condition not supported for paramType \`${ParameterType[condition.paramType]}\``);
    }
};
const checkCompValueIntegrity = (condition) => {
    if (condition.operator >= Operator.EqualTo && !condition.compValue) {
        throw new Error(`\`${Operator[condition.operator]}\` condition must have a compValue`);
    }
    if (condition.operator < Operator.EqualTo && condition.compValue) {
        throw new Error(`\`${Operator[condition.operator]}\` condition cannot have a compValue`);
    }
};
const checkChildrenIntegrity = (condition) => {
    if (condition.paramType === ParameterType.Tuple ||
        ...
        if (!condition.children || condition.children.length === 0) {
            throw new Error(`Condition on \`${ParameterType[condition.paramType]}\` params must have children to describe the type structure, found violation in \`${Operator[condition.operator]}\` condition`);
        }
    }
    if (condition.operator === Operator.ArraySome ||
        condition.operator === Operator.ArrayEvery) {
        if (condition.children?.length !== 1) {
            throw new Error(`\`${Operator[condition.operator]}\` conditions must have exactly one child`);
        }
    }
    if (condition.operator === Operator.Matches ||
        condition.operator === Operator.ArraySubset) {
        if (!condition.children || condition.children.length === 0) {
            throw new Error(`\`${Operator[condition.operator]}\` conditions must have children`);
        }
    }
    if (condition.operator >= Operator.EqualToAvatar &&
        ...
        condition.paramType !== ParameterType.Array) {
        if (condition.children && condition.children?.length > 0) {
            throw new Error(`\`${Operator[condition.operator]}\` condition on \`${ParameterType[condition.paramType]}\` type param must not have children`);
        }
    }
    if (condition.operator === Operator.And ||
        ...
        if (!condition.children || condition.children.length === 0) {
            throw new Error(`\`${Operator[condition.operator]}\` condition must have children`);
        }
    }
};
