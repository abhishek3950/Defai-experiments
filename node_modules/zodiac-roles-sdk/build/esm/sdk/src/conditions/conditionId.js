...
export const conditionId = (condition) => {
    ...
    return concat(conditions.flatMap((condition) => [
 ...
 */
export const conditionAddress = (condition) => {
    const conditions = flattenCondition(condition);
    ...
    return getCreate2Address(ERC2470_SINGLETON_FACTORY_ADDRESS, ZERO_SALT, keccak256(initCode)).toLowerCase();
};
// 8    bits -> parent
// 3    bits -> param type
// 5    bits -> operator
...
const packCondition = (condition) => toBeHex((condition.parent << offsetParent) |
...
const packCompValue = (condition) => {
    ...
    if (!condition.compValue) {
        throw new Error(`compValue is required for operator ${Operator[condition.operator]}`);
    }
...
const initCodeFor = (bytecode) => concat([
    "0x63",
...
const hasCompValue = (operator) => operator >= Operator.EqualTo;
